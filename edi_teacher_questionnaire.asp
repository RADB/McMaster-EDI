		...
		select case Request.Form("frmAction")
			case "_Demographics"				
				...
				for each item in Request.Form 
					...
					elseif NOT (item = "frmEDIYear" OR item = "frmAction" OR item = "frmSection" OR item = "frmSite" OR item ="frmSchool" OR item ="frmTeacher" OR item ="frmClass" OR item = "frmChild" OR item = "frmNextChild" OR item = "DOBday" OR item = "DOByear" OR item = "DOBmonth" OR item = "btnSave" OR item = "hdnLock" OR item = "hdnCheckBoxes" OR item = "hdnRadioButtons" OR item = "Student" OR item = "classes" OR item = "email" OR item ="rpt" OR item ="XML" or item="CurrentSection") then 
						blnUpdate2 = true
						if left(item ,3) = "str" then
			...
			case "A"
				...
				for each item in Request.Form 
					if NOT (item = "frmEDIYear" OR item = "hdnRadioButtons" OR item = "frmAction" OR item = "frmSection" OR item = "frmSite" OR item ="frmSchool" OR item ="frmTeacher" OR item ="frmClass" OR item = "frmChild" OR item = "frmNextChild" OR item = "days2" OR item = "days" OR item = "btnSave" OR item = "hdnLock" OR item = "hdnCheckBoxes" OR item = "Student" OR item = "classes" OR item = "email" OR item ="rpt" OR item ="XML" or item="CurrentSection") then 
						' added May 26 2004
			...
			case "B"
				...
				for each item in Request.Form 
					if NOT (item = "frmEDIYear" OR item = "hdnRadioButtons" OR item = "frmAction" OR item = "frmSection" OR item = "frmSite" OR item ="frmSchool" OR item ="frmTeacher" OR item ="frmClass" OR item = "frmChild" OR item = "frmNextChild" OR item = "btnSave" OR item = "hdnLock" OR item = "hdnCheckBoxes" OR item = "Student" OR item = "classes" OR item = "email" OR item ="rpt" OR item ="XML" or item="CurrentSection") then 
						blnUpdate = true
						if left(item ,3) = "str" then
							strSql = strSql & item & " = " & checknull(Request.Form(item)) & ","
						else
...
'		conn.execute "UPDATE children SET chkCompleted = true, dtmDate = '" & date & "' WHERE strEDIID = '" & strEDIID & "'"	
	...
	<form name="Screens" method="POST" action="edi_teacher_questionnaire.asp"> -->
						...
						case 525, 519,498 ' Questionnaire
							...
							if strError = "" then 
								...
								Response.Write "<table border=""0"" cellpadding=""0"" cellspacing=""0"" width=""750"" align=""left"">"														
									...
									if not blnLock then 
										Response.Write "<a id=""checkLink"" href=""javascript:goConfirm_Lock('" & strEDIYear & "','" & strSite & "','" & strSchool & "','" & strTeacher & "','" & strClass & "','" & strChild & "','lock');"" class=""bigLinkBlue""><img src=""images/lock.gif"" border=""0"" alt=""Lock"" />" & lblCompletion & "</a>"
									end if 
							...
							%>
								<!-- #include virtual="/shared/SectionA.inc" -->
							<%
							'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		<input type="hidden" name="hdnRadioButtons" value="<%=strRadioButtons %>" />
...
function buildRadioButton(orientation,name,title,value,currentvalue,notification,condition)
    strRadio = "&nbsp;&nbsp;<input ondblclick=""this.checked=false"" title=""" & title & """ type=""radio"" value=""" & value & """ id=""" & name & """ name=""" & name & """"
    if currentvalue = value then 
        strRadio = strRadio & " checked=""CHECKED"""
    end if                                      
    strRadio = strRadio & "/>"
    
    if len(strRadioButtons) > 0 then 
        strRadioButtons = strRadioButtons & ","
    end if 
    
	' check to see if the radio button is already listed
	if instr(strRadiobuttons, name & ",") = 0 then 
		strRadioButtons = strRadioButtons & name 
	else
		strRadioButtons = left(strRadioButtons, len(strRadioButtons) - 1)
	end if
    
    buildRadioButton = strRadio    
End function
